...
const API_URL = "https://YOUR_API.mockapi.io/cars";
...
const Home: React.FC = () => {
  ...
  const addCar = useMutation<ICar, Error, ICar>({
    mutationFn: async (newCar) => {
      const res = await axios.post<ICar>(API_URL, newCar);
      return res.data;
    },
    onSuccess: () => {
  ...
  type UpdateVars = { id: string; data: ICar };
  const updateCar = useMutation<ICar, Error, UpdateVars>({
    mutationFn: async ({ id, data }) => {
      const res = await axios.put<ICar>(`${API_URL}/${id}`, data);
      return res.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["cars"] });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      updateCar.mutate({ id: editingId, data: rest as ICar });
    } else {
      const { id, ...rest } = formData;
      addCar.mutate(rest as ICar);
    }
  };

  return (
    <div className="min-h-[90vh] bg-green-50 flex items-start justify-center p</h1>x-6 py-10">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 w-full max-w-6xl">
        <div className="bg-white rounded-xl shadow-md p-8 border border-green-200">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            {(["name", "price", "brand", "color", "release_date", "power"] as (keyof ICar)[]).map(
              (field) => (
                <input
                  key={field}
                  name={field}
        ...
        <div className="bg-green-100 rounded-xl shadow-inner p-8 border border-green-200">
          <h2 className="text-xl font-semibold mb-4 text-green-700">Kiritilgan ma'lumotlar</h2>
          <ul className="space-y-2 text-green-800">
            <li><b>Nomi:</b> {formData.name || "-"}</li>
            <li><b>Narxi:</b> {formData.price || "-"}</li>
            <li><b>Brendi:</b> {formData.brand || "-"}</li>
            <li><b>Rangi:</b> {formData.color || "-"}</li>
            <li><b>Sana:</b> {formData.release_date || "-"}</li>
            <li><b>Quvvati:</b> {formData.power || "-"}</li>
          </ul>
        </div>
      </div>
    </div>
